...
def create_review(db: Session, review: ReviewCreate, user_id: str) -> Optional[Review]:
    ...
    update_car_wash_rating(db, review.car_wash_id)
...
def get_review_stats(db: Session, car_wash_id: str) -> dict:
    ...
    stats = db.query(
        ...
        func.count(func.nullif(Review.nota, 5)).label('nota_5'),
        func.count(func.nullif(Review.nota, 4)).label('nota_4'),
        func.count(func.nullif(Review.nota, 3)).label('nota_3'),
        func.count(func.nullif(Review.nota, 2)).label('nota_2'),
        func.count(func.nullif(Review.nota, 1)).label('nota_1'),
    ).filter(Review.car_wash_id == car_wash_id).first()

    return {
        'nota_media': round(stats.nota_media or 0, 1),
        'total_avaliacoes': stats.total_avaliacoes or 0,
        'distribuicao': {
            '5': stats.nota_5 or 0,
            '4': stats.nota_4 or 0,
            '3': stats.nota_3 or 0,
...
def delete_review(db: Session, review_id: str, user_id: str) -> bool:
    ...
    # Atualiza a nota média do lava-jato
    from app.crud.car_wash import update_car_wash_rating
    update_car_wash_rating(db, car_wash_id)
